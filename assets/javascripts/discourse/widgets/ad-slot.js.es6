...
createWidget("ad-slot", {
  tagName: "",

  html(attrs) {
    const adsToShow = this.adComponents();

    let output = [];

    adsToShow.forEach(widgetName => {
      output.push(this.attach(widgetName, this.attrs));
    });
  ...
  adComponents() {
    const placement = this.attrs.placement,
     availableAdTypes = this.availableAdTypes();
  ...
  isNthPost(n) {
    if (this.attrs.postNumber && n && n > 0) {
      return this.attrs.postNumber % n === 0;
    } else {
      return false;
    }
  }
